...
CONFIG = {
    ...
    "HIDDEN_1": 300,
    "HIDDEN_2": 100,
    "LINEAR_HIDDEN": 100,  # 注意：需保持 HIDDEN_2 与 LINEAR_HIDDEN 一致，以便残差相加
    "GAT_HEADS": 2,  # [修改 2] 新增 GAT 头数配置
    "SEED": 42,
    "TARGET_RUN": 0,  # 指定使用 k_sets 中的第几组数据 (0-9)
    "TARGET_FOLD": 0,  # 指定使用第几折 (0-4)
    "NOTE": "Single Run Test with GAT",
}
...
class Net(torch.nn.Module):
    def __init__(self):
        ...
        # [修改 3] 定义 GAT 层
        # 第一层 GAT: 输入 64 -> 输出 HIDDEN_1 * HEADS
        self.conv1 = GATConv(
            64,
            CONFIG["HIDDEN_1"],
            heads=CONFIG["GAT_HEADS"],
            dropout=CONFIG["DROPOUT_FEAT"],
        )

        # 第二层 GAT: 输入 HIDDEN_1 * HEADS -> 输出 HIDDEN_2
        # 注意：这里设置 heads=1 或者 concat=False，是为了让输出维度等于 HIDDEN_2，
        # 从而可以和下面的 lin1, lin2 进行相加操作。
        self.conv2 = GATConv(
            CONFIG["HIDDEN_1"] * CONFIG["GAT_HEADS"],
            CONFIG["HIDDEN_2"],
            heads=1,
            concat=False,
            dropout=CONFIG["DROPOUT_FEAT"],
        ...
        # 第三层 GAT: 输出层，映射到 1 维
        self.conv3 = GATConv(
            CONFIG["HIDDEN_2"], 1, heads=1, concat=False, dropout=CONFIG["DROPOUT_FEAT"]
        )

        # 线性层保持不变，但需确保 LINEAR_HIDDEN == HIDDEN_2
    ...
    def forward(self):
        ...
        # Conv1 -> ELU (GAT通常配合ELU，但为了保持逻辑一致这里用ReLU，也可换成F.elu)
        ...
        # 残差连接 / 跳跃连接
        # x1 维度是 [N, HIDDEN_2], lin1(x0) 维度是 [N, LINEAR_HIDDEN]
        # 必须保证 HIDDEN_2 == LINEAR_HIDDEN
        x = x1 + torch.relu(self.lin1(x0))
        z = x1 + torch.relu(self.lin2(x0))

        # 重构损失 (Link Prediction Loss)
        pos_loss = -torch.log(
            torch.sigmoid((z[E[0]] * z[E[1]]).sum(dim=1)) + 1e-15
        ).mean()

        # 注意：这里的 13627 和 504378 是硬编码的节点数/边数
...
# --- [MLflow Add] 设置实验名称 ---
mlflow.set_experiment("CPDB_GAT_Experiment")
