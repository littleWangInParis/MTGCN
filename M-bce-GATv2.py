import sys
import numpy as np
import pandas as pd
import time
import pickle
import random
import os
from tqdm import tqdm
from datetime import datetime

import torch
import torch.nn.functional as F
from torch.nn import Linear


# --- [修改 1] 导入 GATv2Conv ---
from torch_geometric.nn import GATv2Conv
from torch_geometric.utils import (
    dropout_edge,
    negative_sampling,
    remove_self_loops,
    add_self_loops,
)

from sklearn import metrics
import mlflow
import mlflow.pytorch

# ==========================================
# 1. 参数配置 (针对 GAT 进行了调整)
# ==========================================
CONFIG = {
    "EPOCH": 2500,
    "LR": 0.01,  # GAT 通常需要比 GCN/Cheb 更小的学习率，建议 0.0005 或 0.001
    "DROPOUT_EDGE": 0.5,
    "DROPOUT_FEAT": 0.5,
    "DROPOUT_ATT": 0.6,  # [新增] 注意力系数的 Dropout，通常设高一点防止过拟合
    "HIDDEN_1": 64,  # 注意：因为有多头，实际输出维度是 HIDDEN_1 * HEADS
    "HIDDEN_2": 32,
    "HEADS": 4,  # [新增] 多头注意力的头数
    "LINEAR_HIDDEN": 100,
    "SEED": 42,
    "TARGET_RUN": 0,
    "TARGET_FOLD": 0,
    "NOTE": "GATv2 Optimization Run with higerer lr and bce",
    "EXP_NAME": 251215,
}


# ==========================================
# 2. 工具函数
# ==========================================
def set_seed(seed):
    if seed is not None:
        random.seed(seed)
        np.random.seed(seed)
        torch.manual_seed(seed)
        if torch.cuda.is_available():
            torch.cuda.manual_seed(seed)
            torch.cuda.manual_seed_all(seed)
        print(f"Random seed set to: {seed}")


def log_single_result(config, auc, aupr, duration):
    filename = "tuning_single_run_log.csv"
    record = config.copy()
    record["Test_AUC"] = auc
    record["Test_AUPR"] = aupr
    record["Duration_Sec"] = round(duration, 2)
    record["Timestamp"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    df_new = pd.DataFrame([record])
    if not os.path.exists(filename):
        df_new.to_csv(filename, index=False)
    else:
        df_new.to_csv(filename, mode="a", header=False, index=False)
    print(f"\n[Result Saved] Results appended to {filename}")


set_seed(CONFIG["SEED"])

# ==========================================
# 3. 数据加载
# ==========================================
if not torch.backends.mps.is_available():
    device = torch.device("mps")
    print("Using Apple Silicon (MPS)")
elif torch.cuda.is_available():
    device = torch.device("cuda")
    print("Using CUDA")
else:
    device = torch.device("cpu")
    print("Using CPU")

print("Loading data...")
if not os.path.exists("./CPDB_data_v2.pt"):
    print("Error: Data file './CPDB_data_v2.pt' not found.")
    sys.exit(1)

data = torch.load("./CPDB_data_v2.pt", weights_only=False)

y_all = np.logical_or(data.y, data.y_te)
Y = torch.tensor(y_all).type(torch.float32).to(device)

if not isinstance(data.mask, torch.Tensor):
    mask_tensor = torch.tensor(data.mask)
    mask_te_tensor = torch.tensor(data.mask_te)
else:
    mask_tensor = data.mask
    mask_te_tensor = data.mask_te

mask_all = (mask_tensor | mask_te_tensor).to(device)

# 特征处理
data.x = data.x[:, :48]
if os.path.exists("./data/str_fearures.pkl"):
    datas = torch.load("./data/str_fearures.pkl", map_location="cpu")
    data.x = torch.cat((data.x, datas), 1)
else:
    print("Warning: './data/str_fearures.pkl' not found, skipping feature concat.")

data.x = data.x.float()
data = data.to(device)

if os.path.exists("./data/k_sets.pkl"):
    with open("./data/k_sets.pkl", "rb") as handle:
        k_sets = pickle.load(handle)
else:
    print("Error: './data/k_sets.pkl' not found.")
    sys.exit(1)

# 处理自环
pb, _ = remove_self_loops(data.edge_index)
pb, _ = add_self_loops(pb)
E = data.edge_index

# 获取动态的节点数和边数，用于负采样
num_nodes = data.x.size(0)
num_edges = pb.size(1)


# ==========================================
# 4. 模型定义 (GATv2 Version)
# ==========================================
class GATNet(torch.nn.Module):
    def __init__(self):
        super(GATNet, self).__init__()

        # 输入特征维度
        in_channels = data.x.size(1)  # 应该是 64

        # --- Layer 1 ---
        # GATv2Conv
        # 输入: 64 -> 输出: HIDDEN_1 * HEADS
        self.conv1 = GATv2Conv(
            in_channels,
            CONFIG["HIDDEN_1"],
            heads=CONFIG["HEADS"],
            dropout=CONFIG["DROPOUT_ATT"],  # 注意力系数的 dropout
            concat=True,  # 中间层通常拼接多头结果
        )

        # --- Layer 2 ---
        # 输入: HIDDEN_1 * HEADS -> 输出: HIDDEN_2 * HEADS
        self.conv2 = GATv2Conv(
            CONFIG["HIDDEN_1"] * CONFIG["HEADS"],
            CONFIG["HIDDEN_2"],
            heads=CONFIG["HEADS"],
            dropout=CONFIG["DROPOUT_ATT"],
            concat=True,
        )

        # --- Layer 3 (Output) ---
        # 输入: HIDDEN_2 * HEADS -> 输出: 1
        # 最后一层通常 concat=False (取平均)，或者 heads=1
        self.conv3 = GATv2Conv(
            CONFIG["HIDDEN_2"] * CONFIG["HEADS"],
            1,
            heads=1,
            dropout=CONFIG["DROPOUT_ATT"],
            concat=False,
        )

        # 线性层保持不变，用于辅助 Loss 计算
        self.lin1 = Linear(in_channels, CONFIG["LINEAR_HIDDEN"])
        self.lin2 = Linear(in_channels, CONFIG["LINEAR_HIDDEN"])

        # 这里的维度对齐需要注意：
        # 原代码中 x1 是 conv2 的输出。
        # 原代码 x1 维度是 HIDDEN_2 (100)。
        # 现在 conv2 输出维度是 HIDDEN_2 * HEADS。
        # 为了让 x1 + lin1(x0) 能相加，我们需要将 lin1 的输出维度调整，或者加一个投影层。
        # 方案：修改 lin1 和 lin2 的输出维度以匹配 conv2 的输出

        self.gat_out_dim = CONFIG["HIDDEN_2"] * CONFIG["HEADS"]

        # 重新定义 Linear 层以匹配 GAT 的输出维度
        self.lin1 = Linear(in_channels, self.gat_out_dim)
        self.lin2 = Linear(in_channels, self.gat_out_dim)

        self.c1 = torch.nn.Parameter(torch.Tensor([0.5]))
        self.c2 = torch.nn.Parameter(torch.Tensor([0.5]))

    def forward(self):
        # Edge Dropout
        edge_index, _ = dropout_edge(
            data.edge_index,
            p=CONFIG["DROPOUT_EDGE"],
            force_undirected=True,
            training=self.training,
        )

        x0 = F.dropout(data.x, p=CONFIG["DROPOUT_FEAT"], training=self.training)

        # Layer 1
        x = self.conv1(x0, edge_index)
        x = F.elu(x)  # GAT 论文推荐使用 ELU 激活函数
        x = F.dropout(x, p=CONFIG["DROPOUT_FEAT"], training=self.training)

        # Layer 2
        x1 = self.conv2(x, edge_index)
        x1 = F.elu(x1)

        # Residual / Skip Connections (保持你原有的逻辑)
        # 注意：现在 x1 的维度是 HIDDEN_2 * HEADS
        x_skip = x1 + F.elu(self.lin1(x0))
        z = x1 + F.elu(self.lin2(x0))

        # --- Loss Calculation ---
        pos_loss = -torch.log(
            torch.sigmoid((z[E[0]] * z[E[1]]).sum(dim=1)) + 1e-15
        ).mean()

        # [修改] 使用动态获取的节点数和边数
        neg_edge_index = negative_sampling(pb, num_nodes, num_edges)

        neg_loss = -torch.log(
            1
            - torch.sigmoid((z[neg_edge_index[0]] * z[neg_edge_index[1]]).sum(dim=1))
            + 1e-15
        ).mean()

        r_loss = pos_loss + neg_loss

        # Layer 3 (Output)
        # 使用 x_skip 继续向前传播
        x_out = F.dropout(x_skip, p=CONFIG["DROPOUT_FEAT"], training=self.training)
        x_out = self.conv3(x_out, edge_index)

        return x_out, r_loss, self.c1, self.c2


# ==========================================
# 5. 训练与测试函数 (保持不变)
# ==========================================
def train(mask, model, optimizer, pos_weight):
    model.train()
    optimizer.zero_grad()
    pred, rl, c1, c2 = model()

    main_loss = F.binary_cross_entropy_with_logits(
        pred[mask], Y[mask], pos_weight=pos_weight
    )

    loss = main_loss / (c1 * c1) + rl / (c2 * c2) + 2 * torch.log(c2 * c1)
    loss.backward()
    optimizer.step()
    return loss.item()


@torch.no_grad()
def test(mask, model):
    model.eval()
    x, _, _, _ = model()
    pred = torch.sigmoid(x[mask]).cpu().detach().numpy()
    Yn = Y[mask].cpu().numpy()
    precision, recall, _ = metrics.precision_recall_curve(Yn, pred)
    area = metrics.auc(recall, precision)
    return metrics.roc_auc_score(Yn, pred), area


# ==========================================
# 6. 主程序
# ==========================================
mlflow.set_experiment(CONFIG["EXP_NAME"])

with mlflow.start_run():
    mlflow.log_params(CONFIG)

    target_run = CONFIG["TARGET_RUN"]
    target_fold = CONFIG["TARGET_FOLD"]
    print(f"Selecting Data: Run {target_run}, Fold {target_fold}")

    _, _, tr_mask, te_mask = k_sets[target_run][target_fold]
    tr_mask = torch.as_tensor(tr_mask).to(device)
    te_mask = torch.as_tensor(te_mask).to(device)

    # ==========================================
    # [新增] 计算 pos_weight
    # ==========================================
    train_labels = Y[tr_mask]
    num_pos = train_labels.sum()
    num_neg = (train_labels == 0).sum()

    # 避免除以0
    if num_pos == 0:
        weight_value = 1.0
    else:
        weight_value = num_neg / num_pos

    # 转为 Tensor 并放入 GPU/CPU
    pos_weight = torch.tensor([weight_value]).to(device)

    print(f"Class Imbalance Calculation: Pos={num_pos}, Neg={num_neg}")
    print(f"Using pos_weight: {weight_value:.4f}")

    # 记录到 MLflow 方便后续分析
    mlflow.log_param("pos_weight", weight_value.item())
    # ==========================================

    # 使用新的 GATNet
    model = GATNet().to(device)

    # 可以在这里加一个 Weight Decay (L2正则)，GAT 对此比较敏感
    optimizer = torch.optim.Adam(model.parameters(), lr=CONFIG["LR"], weight_decay=5e-4)

    time_start = time.time()
    loop = tqdm(range(1, CONFIG["EPOCH"] + 1), desc="Training GAT", dynamic_ncols=True)

    best_auc = 0
    best_epoch = 0

    for epoch in loop:
        loss_val = train(tr_mask, model, optimizer, pos_weight)
        mlflow.log_metric("train_loss", loss_val, step=epoch)

        if epoch % 100 == 0:
            val_auc, val_aupr = test(te_mask, model)
            mlflow.log_metrics({"val_auc": val_auc, "val_aupr": val_aupr}, step=epoch)

            if val_auc > best_auc:
                best_auc = val_auc
                best_epoch = epoch

            loop.set_postfix(loss=f"{loss_val:.4f}", val_auc=f"{val_auc:.4f}")
            model.train()

    final_auc, final_aupr = test(te_mask, model)
    total_duration = time.time() - time_start

    print("\n" + "=" * 30)
    print("GAT RUN RESULT")
    print("=" * 30)
    print(f"Final Test AUC : {final_auc:.5f}")
    print(f"Final Test AUPR: {final_aupr:.5f}")
    print(f"Best AUC seen  : {best_auc:.5f} (at epoch {best_epoch})")
    print("=" * 30)

    mlflow.log_metrics(
        {
            "final_test_auc": final_auc,
            "final_test_aupr": final_aupr,
            "best_val_auc": best_auc,
            "duration_seconds": total_duration,
        }
    )

    mlflow.pytorch.log_model(model, "model")
    log_single_result(CONFIG, final_auc, final_aupr, total_duration)
